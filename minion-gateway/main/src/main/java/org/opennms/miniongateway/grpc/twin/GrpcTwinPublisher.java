            ...
            if (Strings.isNullOrEmpty(twinResponseProto.getLocation())) {
                ...
                for (Entry<TenantKey, AdapterObserver> entry : new ArrayList<>(sinkStreamsByLocation.entries())) {
                    if (tenantId.equals(entry.getKey().getTenantId())) {
                        ...
                        try {
                            ...
                            LOG.debug("Sending sink update for key {} and system id {}", twinResponseProto.getConsumerKey(), twinResponseProto.getSystemId());
                ...
                for (AdapterObserver stream : new ArrayList<>(observers)) {
                    try {
                        ...
                        stream.complete();
                        LOG.debug("Failed to send sink update for key {} at location {}", twinResponseProto.getConsumerKey(), twinResponseProto.getLocation());
                    }
                }
            }
        } catch (Exception e) {
