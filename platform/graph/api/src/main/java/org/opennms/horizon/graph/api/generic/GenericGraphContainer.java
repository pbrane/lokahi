...
import org.opennms.horizon.graph.api.validation.GraphContainerIdValidator;
...
public class GenericGraphContainer implements ImmutableGraphContainer<GenericGraph> {
    ...
    private GenericGraphContainer(GenericGraphContainerBuilder builder) {
        ...
        this.graphs = ImmutableList.copyOf(builder.graphs.values()
            .stream().sorted(Comparator.comparing(GenericGraph::getNamespace))
    ...
    public GraphInfo getGraphInfo(String namespace) {
        ...
        return graphs.stream().filter(g -> g.getNamespace()
            .equals(namespace)).findAny().orElse(null);
    }
