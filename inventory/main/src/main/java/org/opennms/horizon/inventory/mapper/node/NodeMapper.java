...
public class NodeMapper {
    ...
    public List<NodeDTO> modelToDto(List<Node> list) {
        return list.stream().map(this::modelToDto).toList();
    }

    public NodeDTO modelToDto(Node node) {
        NodeDTO.Builder builder = NodeDTO.newBuilder();
        if (node instanceof DefaultNode defaultNode) {
            builder.setDefault(defaultNodeMapper.modelToDto(defaultNode));
        } else if (node instanceof AzureNode azureNode) {
            builder.setAzure(azureNodeMapper.modelToDto(azureNode));
        }
