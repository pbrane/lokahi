...
@Mapper(componentModel = "spring", uses = {EmptyStringMapper.class, IpInterfaceMapper.class},
    // Needed for grpc proto mapping
    collectionMappingStrategy = CollectionMappingStrategy.ADDER_PREFERRED,
    nullValueCheckStrategy = NullValueCheckStrategy.ALWAYS)
public interface AzureNodeMapper extends DateTimeMapper {

    @Mapping(source = "ipInterfaces", target = "ipInterfacesList")
    @Mapping(source = "monitoringLocation.location", target = "monitoringLocation")
    AzureNodeDTO modelToDto(AzureNode azureNode);
}
