...
public class SnmpStore extends AbstractSnmpStore {
     ...
     *
     * @param list an array of {@link NamedSnmpVar} objects.
     */
     ...
     * <p>getElements</p>
     *
     * @return an array of {@link NamedSnmpVar} objects.
     */
    ...
    public void storeResult(final SnmpResult res) {
        ...
        for (final NamedSnmpVar var : ms_elemList) {
            if (base.equals(var.getSnmpObjId())) {
                if (value.isError()) {
                    LOG.error(
                            "storeResult: got an error for alias {} [{}].[{}], but we should only be getting non-errors: {}",
                            var.getAlias(),
                            base,
                            res.getInstance(),
                            value);
