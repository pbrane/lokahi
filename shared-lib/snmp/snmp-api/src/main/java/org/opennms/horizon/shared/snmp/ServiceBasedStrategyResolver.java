...
public class ServiceBasedStrategyResolver implements StrategyResolver {
    ...
    public ServiceBasedStrategyResolver() {}

    public ServiceBasedStrategyResolver(SnmpStrategy snmpStrategy) {
        this.m_strategies.put(snmpStrategy.getClass().getName(), snmpStrategy);
    }
    ...
    public void onBind(SnmpStrategy strategy, Map<String, String> props) {
        ...
        if (key == null) {
            LOG.error(
                    "SnmpStrategy class '{}' published as service with out 'implementation' key.  Ignoring.",
                    strategy.getClass());
            return;
        }
    ...
    public SnmpStrategy getStrategy() {
        ...
        if (strategy == null) {
            if (m_strategies.isEmpty()) {
                LOG.warn("There is no SnmpStrategy registered. Unable to find strategy " + strategyClass
                        + ". Falling back to ClassBasedStrategyResolver.");
                return s_classBasedStrategyResolver.getStrategy();
            } else {
                Map.Entry<String, SnmpStrategy> entry =
                        m_strategies.entrySet().iterator().next();
