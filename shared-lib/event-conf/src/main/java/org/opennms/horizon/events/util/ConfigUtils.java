...
public class ConfigUtils {
        ...
        if (value instanceof String) {
            if (!in.contains(value)) {
                throw new IllegalStateException(
                        "'" + name + "': found '" + value.toString() + "' but expected one of: " + in.toString());
            }
            ...
            for (final K entry : col) {
                if (!in.contains(entry)) {
                    throw new IllegalStateException(
                            "'" + name + "': found '" + entry.toString() + "' but expected one of: " + in.toString());
                }
            }
        } else {
    ...
    public static String assertMatches(final String value, final Pattern pattern, final String name) {
        ...
        if (!m.matches()) {
            throw new IllegalStateException(
                    "'" + name + "': value '" + value + "' did no match pattern '" + pattern + "'");
        }
        return value;
    }
}
