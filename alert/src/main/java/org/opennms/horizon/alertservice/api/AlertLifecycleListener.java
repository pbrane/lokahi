...
/**
 * A simplified version of the {@link AlertEntityListener} interface with support
...
public interface AlertLifecycleListener {
    ...
    /**
     * Called periodically with a complete set of alerts as present in the database
     * at the given timestamp.
     *
     * This should be used to synchronize any state to ensure it matches what is currently
     * in the database.
     *
     * Note that it is possible that the *current* state of alerts is different from the state
     ...
     * the alert are still open.
     *
     * All of the listeners are invoked serially, so the implementors should avoid
     * blocking when possible.
     *
     * @param alerts canonical set of alerts in the database
     */
    void handleAlertSnapshot(List<Alert> alerts);

    /**
     * Called before the transaction is opened and the alerts are read for subsequent
     * calls to {@link #handleAlertSnapshot}.
     *
     * This can be used to trigger any necessary state tracking to accurately handle
     * the snapshot results.
     */
    void preHandleAlertSnapshot();

    /**
     * Called after {@link #handleAlertSnapshot} has been called on all the listeners, and
     ...
     *
     * This function may be called immediately after a call to {@link #preHandleAlertSnapshot} if
     * an error occurred while preparing the snapshot i.e. when opening the transaction.
     */
    void postHandleAlertSnapshot();
     ...
     * blocking when possible.
     *
     * @param alertId id of the alert that was deleted
     * @param reductionKey reduction key of the alert that was deleted
     */
    void handleDeletedAlert(long alertId, String reductionKey);
