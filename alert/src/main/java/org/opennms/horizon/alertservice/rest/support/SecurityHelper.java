...
package org.opennms.horizon.alertservice.rest.support;
...
//        throw new WebApplicationException(Response.status(Response.Status.FORBIDDEN).entity("User '" + currentUser + "', is not allowed to read alerts.").type(MediaType.TEXT_PLAIN).build());
    }
...
//        }
//        // otherwise
//        throw new WebApplicationException(Response.status(Response.Status.FORBIDDEN).entity("User '" + currentUser + "', is not allowed to perform updates to alerts as user '" + ackUser + "'").type(MediaType.TEXT_PLAIN).build());
    }
