...
Inventory refers to the devices and entities in your network that OpenNMS Cloud has identified through the xref:discover/introduction.adoc[discovery] process.
...
They did not match any of the criteria in any monitoring policy, including the xref:policies/introduction.adoc#default-monitoring-policy[default one].
* *Detected Nodes*: Nodes identified through an indirect way like via flow.
You can choose to monitor the node through a xref:policies/introduction.adoc[monitoring policy], or ignore it.
