        ...
        <section class="feather-row">
          <div class="feather-col-12">
            <div class="alert-rules-header">
              ...
              <p class="sub-title">Create one or more alert rules to define your alert triggers. Monitoring Policies must include at least one Alert Rule.</p>
            </div>
            <div class="alerts-rule" v-if="!isCheckAlertRule">
              <MonitoringPoliciesAlertsRulesTable/>
            </div>
            <div v-else class="alerts-rule">
...
const isCheckAlertRule = computed(() => {
  return (monitoringPoliciesStore.selectedPolicy?.rules?.length ?? 0 ) <= 0
})
...
const savePolicy = async () => {
  await monitoringPoliciesStore?.savePolicy()
  router.push('/monitoring-policies-new/')
}
</script>
...
.content {
  .policies-content {
    ...
    .alerts-rule,.alert-rules-header {
