...
const disableSaveRuleBtn = computed(
  () => store.selectedPolicy?.isDefault || !store.selectedRule?.name || !store.selectedRule?.alertConditions.length || store.selectedRule.alertConditions.filter((alertCondition) => !alertCondition.triggerEvent).length
)
</script>
