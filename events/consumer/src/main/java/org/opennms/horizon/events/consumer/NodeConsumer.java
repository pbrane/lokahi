...
public class NodeConsumer {
    ...
    @Autowired
    public NodeConsumer(EventService eventService) {
        this.eventService = eventService;
    }

    @KafkaListener(topics = "${kafka.node-changed}", concurrency = "1")
            ...
            switch (nodeOperation.getOperationValue()) {
                case NodeOperation.UPDATE_NODE_VALUE:
                    Context.current()
                            ...
                            .run(() -> eventService.saveNode(nodeOperation.getNodeDto()));
                    break;
                ...
                case NodeOperation.REMOVE_NODE_VALUE:
                    Context.current()
                            .withValue(
                                    GrpcConstants.TENANT_ID_CONTEXT_KEY,
                                    nodeOperation.getNodeDto().getTenantId())
                            .run(() -> eventService.deleteAllEventByNodeId(nodeOperation));
                    break;
