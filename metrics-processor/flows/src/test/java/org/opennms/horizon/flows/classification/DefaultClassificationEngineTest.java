...
public class DefaultClassificationEngineTest {
    private static final Protocol TCP = Protocols.getProtocol("tcp");
    private static final Protocol UDP = Protocols.getProtocol("udp");

    private static ClassificationRequest classificationRequest(
            String location, int srcPort, String srcAddress, int dstPort, String dstAddress, Protocol protocol) {
        return new ClassificationRequest(
                location, srcPort, IpAddr.of(srcAddress), dstPort, IpAddr.of(dstAddress), protocol);
    }
    ...
    public void verifyRuleEngineBasic() throws InterruptedException {
        ...
        assertEquals(
                "rule2",
                engine.classify(new ClassificationRequestBuilder()
                        .withSrcPort(9999)
                        .withDstPort(443)
                        .build()));
        assertEquals(
    ...
    public void verifyRuleEngineWithOmnidirectionals() throws InterruptedException {
        ...
        assertEquals(
                ...
                engine.classify(new ClassificationRequestBuilder()
                        ...
                        .withDstPort(9999)
                        .build()));
        assertNull(engine.classify(new ClassificationRequestBuilder()
                .withSrcPort(9999)
                .withDstPort(1337)
                .build()));
    ...
    public void verifyRuleEngineExtended() throws InterruptedException {
        ...
        DefaultClassificationEngine engine = new DefaultClassificationEngine(
                () -> Lists.newArrayList(
                        ...
                        new RuleBuilder()
                                .withName("DUMMY")
                                .withDstAddress("192.168.1.0-192.168.1.255,10.10.5.3,192.168.0.0/24")
                                .withDstPort("8000-9000,80,8080")
                                .withPosition(4)
                                .build(),
                        new RuleBuilder()
                                .withName("RANGE-TEST")
                                .withDstPort("7000-8000")
                                .withPosition(5)
                                .build(),
                        new RuleBuilder()
                                .withName("OpenNMS")
        ...
        assertEquals("SSH", engine.classify(classificationRequest("Default", 0, null, 22, "127.0.0.1", TCP)));
        assertEquals("HTTP_CUSTOM", engine.classify(classificationRequest("Default", 0, null, 80, "192.168.0.1", TCP)));
        assertEquals("HTTP", engine.classify(classificationRequest("Default", 0, null, 80, "192.168.0.2", TCP)));
        assertEquals(null, engine.classify(classificationRequest("Default", 0, null, 5000, "localhost", UDP)));
        assertEquals(null, engine.classify(classificationRequest("Default", 0, null, 5000, "localhost", TCP)));
        assertEquals("OpenNMS", engine.classify(classificationRequest("Default", 0, null, 8980, "127.0.0.1", TCP)));
        assertEquals(
                "OpenNMS Monitor",
                engine.classify(new ClassificationRequestBuilder()
        ...
        for (var ipAddress : ipAddresses) {
            final ClassificationRequest classificationRequest =
                    new ClassificationRequest("Default", 0, null, 8080, ipAddress, TCP);
        ...
        // Verify Port Range
        IntStream.range(7000, 8000)
                .forEach(i -> assertEquals(
                        "RANGE-TEST",
                        engine.classify(classificationRequest("Default", 0, null, i, "192.168.0.2", TCP))));
    ...
    public void verifyAddressRuleWins() throws InterruptedException {
        final ClassificationEngine engine = new DefaultClassificationEngine(
                () -> Lists.newArrayList(
                        new RuleBuilder()
                                .withName("HTTP")
                                .withDstPort(80)
        ...
        final ClassificationRequest classificationRequest =
                classificationRequest("Default", 0, null, 80, "192.168.2.1", TCP);
    ...
    public void verifyAllPortsToEnsureEngineIsProperlyInitialized() throws InterruptedException {
        final ClassificationEngine classificationEngine =
                new DefaultClassificationEngine(() -> new ArrayList<>(), FilterService.NOOP);
        for (int i = Rule.MIN_PORT_VALUE; i < Rule.MAX_PORT_VALUE; i++) {
            classificationEngine.classify(classificationRequest("Default", 0, null, i, "127.0.0.1", TCP));
        }
    }
    ...
    public void verifyDoesNotRunOutOfMemory() throws InterruptedException {
        ...
        for (int i = 0; i < 100; i++) {
            final Rule rule = new RuleBuilder()
                    ...
                    .withPosition(i + 1)
                    .withProtocol("UDP")
                    .withDstAddress("192.168.0." + i)
                    .build();
            rules.add(rule);
        }
        final DefaultClassificationEngine engine = new DefaultClassificationEngine(() -> rules, FilterService.NOOP);
        engine.classify(
                classificationRequest("localhost", 1234, "127.0.0.1", 80, "192.168.0.1", Protocols.getProtocol("UDP")));
    }
