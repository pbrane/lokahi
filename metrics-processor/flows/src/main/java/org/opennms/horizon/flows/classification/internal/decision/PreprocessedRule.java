...
import org.opennms.horizon.flows.classification.internal.value.ProtocolValue;
...
public class PreprocessedRule {
    ...
    private static Stream<Threshold> addressThresholds(IpValue value, Function<IpAddr, Threshold> thresholdCreator) {
        return value == null
                ? Stream.empty()
                : value.getIpAddressRanges().stream()
    ...
    // if a rule does not specify a criteria for some aspect then the corresponding ProtocolValue, PortValue, or IpValue
    // is null
    ...
    public PreprocessedRule reverse() {
        return new PreprocessedRule(ruleDefinition.reversedRule(), protocol, dstPort, srcPort, dstAddr, srcAddr);
    }
}
