...
public class V6Pinger extends AbstractPinger<Inet6Address> {
    ...
    public void run() {
        try {
            ...
            while (!isFinished()) {
                ...
                final V6PingReply echoReply =
                        icmpPacket.getType() == Type.EchoReply ? new V6PingReply(icmpPacket, received) : null;

                if (echoReply != null && echoReply.isValid()) {
                    // 64 bytes from 127.0.0.1: icmp_seq=0 time=0.069 ms
                    System.out.printf(
                            "%d bytes from [%s]: tid=%d icmp_seq=%d time=%.3f ms%n",
    ...
    @Override
    public PingReplyMetric ping(
            final Inet6Address addr, final int id, final int sequenceNumber, final int count, final long interval)
