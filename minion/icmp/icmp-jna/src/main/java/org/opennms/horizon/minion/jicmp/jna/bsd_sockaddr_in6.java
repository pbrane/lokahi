...
public class bsd_sockaddr_in6 extends Structure {
    ...
    public bsd_sockaddr_in6(InetAddress address, int port) {
        this(NativeDatagramSocket.AF_INET6, address.getAddress(), new byte[] {
            (byte) (0xff & (port >> 8)), (byte) (0xff & port)
        });
    }

    public bsd_sockaddr_in6(int port) {
        this(NativeDatagramSocket.AF_INET6, new byte[16], new byte[] {(byte) (0xff & (port >> 8)), (byte) (0xff & port)
        });
    }
    ...
    protected List<String> getFieldOrder() {
        return Arrays.asList(
                new String[] {"sin6_len", "sin6_family", "sin6_port", "sin6_flowinfo", "sin6_addr", "sin6_scope_id"});
    }

    private void assertLen(String field, byte[] addr, int len) {
        if (addr.length != len) {
            throw new IllegalArgumentException(
                    field + " length must be " + len + " bytes but was " + addr.length + " bytes.");
        }
    }
