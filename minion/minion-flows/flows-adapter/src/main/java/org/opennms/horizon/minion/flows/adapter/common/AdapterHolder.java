...
public class AdapterHolder implements Listener {

    private Map<String, FlowsListener> adapterMap = new HashMap<>();
    ...
    public void put(FlowsListener listener) {
        this.adapterMap.put(listener.getName(), listener);
    }

    public FlowsListener remove(String name) {
        return this.adapterMap.remove(name);
    }

    @Override
    public void start() throws Exception {
        for (FlowsListener listener : adapterMap.values()) {
            listener.start();
        }
    }
    ...
    @Override
    public void stop() {
        adapterMap.values().forEach(FlowsListener::stop);
    }
}
