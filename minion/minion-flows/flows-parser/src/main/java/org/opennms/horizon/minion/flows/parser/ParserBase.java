...
public abstract class ParserBase implements Parser {
    ...
    private final RateLimitedLog SEQUENCE_ERRORS_LOGGER = RateLimitedLog.withRateLimit(LOG)
            .maxRate(5)
    ...
    public ParserBase(
        ...
        final LogPreservingThreadFactory logPreservingThreadFactory =
    ...
    public void start(ScheduledExecutorService executorService) {
        executor = new ThreadPoolExecutor(
                ...
                1, threads, 60L, TimeUnit.SECONDS, new SynchronousQueue<>(), threadFactory, (r, executor) -> {
                    // We enter this block when the queue is full and the caller is attempting to submit additional
                    // tasks
                    try {
                        ...
                        throw new RejectedExecutionException(
                                "Executor interrupted while waiting for capacity in the work queue.", e);
                    }
                });
    }
    ...
    public void setClockSkewEventRate(final long clockSkewEventRate) {
        ...
        this.clockSkewEventCache = CacheBuilder.newBuilder()
    ...
    public void setIllegalFlowEventRate(final long illegalFlowEventRate) {
        ...
        this.illegalFlowEventCache = CacheBuilder.newBuilder()
    ...
    protected CompletableFuture<?> transmit(
            final RecordProvider packet, final Session session, final InetSocketAddress remoteAddress) {
                ...
                () -> {
                    return packet.getRecords()
                            .map(record -> {
                                ...
                                final RecordEnricher recordEnricher =
                                        new RecordEnricher(dnsResolver, getDnsLookupsEnabled());
                                recordEnricher.enrich(record).whenComplete((enrichment, ex) -> {
                                    ...
                                    final Runnable dispatch = () -> {
                                        ...
                                        try {
                                            flowDocument =
                                                    this.getMessageBuilder().buildMessage(record, enrichment);
                                        } catch (final Exception e) {
                                            throw new RuntimeException(e);
                                        }

                                        flowDocument.setExporterAddress(
                                                InetAddressUtils.str(remoteAddress.getAddress()));
                                        ...
                                        if (!corrections.isEmpty()) {
                                            ...
                                            final Optional<Instant> instant =
                                                    illegalFlowEventCache.getUnchecked(session.getRemoteAddress());

                                            if (instant.isEmpty()
                                                    || Duration.between(instant.get(), Instant.now())
                                                                    .getSeconds()
                                                            > getIllegalFlowEventRate()) {
                                                illegalFlowEventCache.put(
                                                        session.getRemoteAddress(), Optional.of(Instant.now()));

                                                for (final String correction : corrections) {
                                                    LOG.warn(
                                                            "Illegal flow detected from exporter {}: \n{}",
                                    ...
                                    // It's possible that the callback thread is already a thread from the pool, if
                                    // that's the case
    ...
    protected void detectClockSkew(final long packetTimestampMs, final InetAddress remoteAddress) {
        if (getMaxClockSkew() > 0) {
            ...
            if (deltaMs > maxClockSkew) {
                ...
                if (instant.isEmpty()
                        || Duration.between(instant.get(), Instant.now()).getSeconds() > getClockSkewEventRate()) {
    ...
    private List<String> correctFlow(final FlowDocument.Builder flow) {
        ...
        if (flow.getFirstSwitched().getValue() > flow.getLastSwitched().getValue()) {
            corrections.add(String.format(
                    "Malformed flow: lastSwitched must be greater than firstSwitched: srcAddress=%s, dstAddress=%s, firstSwitched=%d, lastSwitched=%d, duration=%d",
                    flow.getSrcAddress(),
                    flow.getDstAddress(),
            ...
            final long timeout = (flow.hasDeltaSwitched()
