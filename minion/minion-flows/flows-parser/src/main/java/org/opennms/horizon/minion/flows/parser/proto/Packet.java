 ...
 * License.
 */
package org.opennms.horizon.minion.flows.parser.proto;
...
public final class Packet implements Iterable<Record>, RecordProvider {
    ...
    public Packet(final Header header, final ByteBuf buffer) throws InvalidPacketException {
        this.header = Objects.requireNonNull(header);

        final List<Record> records = new LinkedList();

        while (buffer.isReadable(Record.SIZE) && records.size() < this.header.count) {
    ...
    public Stream<Iterable<Value<?>>> getRecords() {
        ...
        return this.records.stream().map(Record::asValues).map(record -> Iterables.concat(header, record));
    }
