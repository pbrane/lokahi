...
public class DateTimeValue extends Value<Instant> {
    ...
    public DateTimeValue(final String name, final Optional<Semantics> semantics, final Instant value) {
        ...
        return new InformationElement() {
            ...
            public Value<?> parse(final Session.Resolver resolver, final ByteBuf buffer) {
                return new DateTimeValue(
                        name, semantics, Instant.ofEpochMilli(uint64(buffer).longValue()));
            }
    ...
    public static InformationElement parserWithMicroseconds(final String name, final Optional<Semantics> semantics) {
        return new InformationElement() {
            ...
            public Value<?> parse(final Session.Resolver resolver, final ByteBuf buffer) {
                ...
                final Instant value =
                        Instant.ofEpochSecond(seconds - SECONDS_TO_EPOCH, fraction * 1_000_000_000L / (1L << 32));
