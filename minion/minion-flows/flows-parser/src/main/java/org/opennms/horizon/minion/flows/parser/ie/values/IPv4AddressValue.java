...
public class IPv4AddressValue extends Value<Inet4Address> {
    ...
    public IPv4AddressValue(final String name, final Optional<Semantics> semantics, final Inet4Address value) {
            ...
            public Value<?> parse(final Session.Resolver resolver, final ByteBuf buffer) throws InvalidPacketException {
                try {
                    return new IPv4AddressValue(
                            name, semantics, (Inet4Address) Inet4Address.getByAddress(bytes(buffer, 4)));
