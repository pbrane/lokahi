...
import static org.opennms.horizon.minion.flows.parser.transport.MessageUtils.getDoubleValue;
import static org.opennms.horizon.minion.flows.parser.transport.MessageUtils.getInetAddress;
...
import static org.opennms.horizon.minion.flows.parser.transport.MessageUtils.setDoubleValue;
...
public class Netflow9MessageBuilder implements MessageBuilder {
    ...
    public FlowDocument.Builder buildMessage(final Iterable<Value<?>> values, final RecordEnrichment enrichment) {
        ...
        for (Value<?> value : values) {
            switch (value.getName()) {
                ...
                case "@sequenceNumber":
                    ...
                    break;
                case "@sourceId":
                    getUInt64Value(value)
                            .ifPresent(srcId -> builder.setExporterIdentifier(String.valueOf(srcId.getValue())));
                    break;
                case "@sysUpTime":
        ...
        if (firstSwitched != null) {
            ...
            // Some Cisco platforms also support absolute timestamps in NetFlow v9 (like defined in IPFIX). See
            // NMS-13006
        ...
        // dst mask
        first(ipv6DstMask, dstMask).ifPresent(dstMaskLen -> builder.setDstMaskLen(setIntValue(dstMaskLen.intValue())));
