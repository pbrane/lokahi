...
public class NettyDnsResolver implements DnsResolver {
    ...
    public NettyDnsResolver(MetricRegistry metrics) {
        ...
        metrics.register(MetricRegistry.name("dns", "availableConcurrentCalls"), (Gauge<Integer>)
                () -> bulkhead.getMetrics().getAvailableConcurrentCalls());
        metrics.register(MetricRegistry.name("dns", "maxAllowedConcurrentCalls"), (Gauge<Integer>)
    ...
    public void init() {
        ...
        cache = new CaffeineDnsCache(
                minTtlSeconds < 0 ? cacheWithDefaults.minTtl() : minTtlSeconds,
    ...
    public DnsServerAddressStreamProvider getNameServerProvider() {
        ...
        return new SequentialDnsServerAddressStreamProvider(
                toSocketAddresses(nameservers).toArray(new InetSocketAddress[] {}));
    }
        ...
        return Arrays.stream(servers)
                .map(s -> {
                    final HostAndPort hp =
                            HostAndPort.fromString(s.trim()).withDefaultPort(53).requireBracketsForIPv6();
